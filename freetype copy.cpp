...
unsigned int renderText(float x, float y, float z, uint8 quality, String &s, Vec3 *pos, RgbaColor *col, Vec2* coords, float font_size, float *width, float *height, RgbaColor color){
    ...
    for(int i=0; i<s.length; i++){
        for(int j=0; j<glyphNum; j++)
            if(s.text[i] == glyphs[quality][j].letter){
                if(glyphs[quality][j].size.x > 0)
                    squares += 6;
                break;
            }
    }
...
u32 render_text(float x, float y, float z, u8 quality, String &s, Vertex_PTCa *vertices, float font_size, float *width, float *height, RgbaColor color){
        ...
        for(int j=0; j<glyphNum; j++)
            if(s.text[i] == glyphs[quality][j].letter){
                ...
                if(g.size.x > 0){
                    ...
                    float x1 = x+font_size*g.offset.x, y1 = y+font_size*g.offset.y;
                    float x2 = x1+font_size*g.size.x, y2 = y1+font_size*g.size.y;
                    *(vertices++) = {Vec3(x1, y1, z), Vec2(g.coords1.x, g.coords1.y), color};
                    *(vertices++) = {Vec3(x2, y1, z), Vec2(g.coords2.x, g.coords1.y), color};
                    *(vertices++) = {Vec3(x1, y2, z), Vec2(g.coords1.x, g.coords2.y), color};
                    *(vertices++) = {Vec3(x2, y1, z), Vec2(g.coords2.x, g.coords1.y), color};
                    *(vertices++) = {Vec3(x1, y2, z), Vec2(g.coords1.x, g.coords2.y), color};
                    *(vertices++) = {Vec3(x2, y2, z), Vec2(g.coords2.x, g.coords2.y), color};
                }
...
u32 render_text_monospace(float x, float y, float z, u8 quality, String &s, Vertex_PTCa *vertices, float font_size, float *width, float *height, RgbaColor color){
    Vertex_PTCa *o_vertices = vertices;
    // Render log
    // Count the total squares
    float x0 = x;
...
unsigned int renderText(float x, float y, float z, uint8 quality, String &s, Vec3 *pos, RgbaColor *col, Vec2* coords, float font_size, float *width, float *height){
    return renderText(x, y, z, quality, s, pos, col, coords, font_size, width, height, {0, 0, 0, 155});
}
