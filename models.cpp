...
StaticModel *get_static_model(const char *path, StaticModel *static_model){
    OsFile fp = openGameFile(path, "rb");
    if(fp == nullptr){
        printf("Couldn't read '%s'\n", path);
        return nullptr;
    }
    u32 size;
    readFile(&size, sizeof(u32), 1, fp);
    
    Vec3     positions[3*MAX_MODEL_TRIANGLES];
    Vec3     normals[MAX_MODEL_TRIANGLES];
    RgbColor colors[MAX_MODEL_TRIANGLES];
    readFile(positions, sizeof(Vec3), 3*size, fp);
    readFile(colors, sizeof(RgbColor), size, fp);
    readFile(normals, sizeof(Vec3), size, fp);
    closeFile(fp);
